 ...
 * GrayTonePicker.jsx
 *
 * Selector modal de tonos (50-900)
 * Permite cambiar el tono manteniendo la misma familia de color
 */

import React from 'react';

// Tonos disponibles con descripciones
const TONES = [
  { value: 50, label: '50', desc: 'Muy claro' },
  { value: 100, label: '100', desc: 'Claro' },
  { value: 200, label: '200', desc: 'Claro medio' },
  { value: 300, label: '300', desc: 'Medio claro' },
  { value: 400, label: '400', desc: 'Medio' },
  { value: 500, label: '500', desc: 'Base' },
  { value: 600, label: '600', desc: 'Medio oscuro' },
  { value: 700, label: '700', desc: 'Oscuro' },
  { value: 800, label: '800', desc: 'Muy oscuro' },
  { value: 900, label: '900', desc: 'Casi negro' },
];
...
const getTailwindColor = (family, tone) => {
  ...
  return colors[family]?.[tone] || '#cccccc';
};

const GrayTonePicker = ({
  isOpen,
  onClose,
  onSelect,
  currentTone = 500,
  family = 'neutral',
  title = 'Seleccionar tono de gris'
}) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-surface1 text-on-surface1 rounded-lg shadow-xl max-w-2xl w-full mx-4">
        {/* Header */}
        ...
        <div className="p-6">
          <div className="grid grid-cols-2 gap-3">
            {TONES.map((tone) => {
              const isSelected = tone.value === currentTone;
              const colorHex = getTailwindColor(family, tone.value);

              return (
                <button
                  key={tone.value}
                  onClick={() => handleSelect(tone.value)}
                  className={`
                ...
                >
                  {/* Cuadro de color */}
                  <div
                    className="w-12 h-12 rounded-md shadow-sm border-2 border-white flex-shrink-0"
                  ...
                  {isSelected && (
                    <div className="text-sm font-bold text-on-primary">
                      ✓
                    </div>
                  )}
                </button>
